...
main (int argc, char *argv[])
{
  ...
  label = clutter_text_new_with_text ("Mono 26", "Ubuntu Tweak");
  clutter_actor_set_position (label, 50, 50);
  clutter_actor_show (label);

  error = NULL;
  hand = clutter_texture_new_from_file ("ubuntu-tweak.png",
                                        &error);
  if (error)
  ...
  clutter_actor_set_position (box, 200, 150);
  clutter_actor_set_scale (box, 1, 1);
