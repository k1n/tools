...
static void
deep_copy_async_data_free (DeepCopyAsyncData *data)
{
...
static gboolean
g_file_real_deep_copy (GFile                     *file,
  ...
  if (g_file_info_get_file_type (info) == G_FILE_TYPE_DIRECTORY)
    {
      ...
      if (relative_path == NULL) 
      {
	      ...
	      target = g_file_resolve_relative_path (target, relative_path);
	      g_file_make_directory (target, cancellable, error);
      } else {
	      child = g_file_resolve_relative_path (target, relative_path);
	      g_file_make_directory (child, cancellable, error);
      }
              ...
              while (!g_cancellable_is_cancelled (cancellable) && success)
                {
                  ...
                  success = success && g_file_real_deep_copy (child, cancellable, progress_callback, progress_callback_data, progress_data, error);
                   ...
                   GError                   **error)
{
  ...
  return g_file_real_deep_copy (file, cancellable, progress_callback, progress_callback_data, &data, error);
}



static gboolean
deep_copy_async_progress_in_main (gpointer user_data)
{
  ProgressData       *progress = user_data;
  DeepCopyAsyncData *data = progress->data;
...
static void
deep_copy_async_progress_callback (goffset  current_num_files,
                                    goffset  current_num_bytes,
                                    gpointer user_data)
{
  DeepCopyAsyncData *data = user_data;
  ...
  g_io_scheduler_job_send_to_mainloop_async (data->job, deep_copy_async_progress_in_main, progress, g_free);
}



static gboolean
deep_copy_async_thread (GIOSchedulerJob *job,
                         GCancellable    *cancellable,
                         gpointer         user_data)
{
  ...
  data->job = job;
  result = g_file_deep_copy (data->file, cancellable, data->progress_cb != NULL ? deep_copy_async_progress_callback : NULL, data, &error);
...
static void
g_file_deep_copy_async (GFile                     *source,
                         int                        io_priority,
                         GCancellable              *cancellable,
                         GFileCountProgressCallback progress_callback,
                         gpointer                   progress_callback_data,
                         ...
                         gpointer                   callback_data)
{
  ...
  data = g_new0 (DeepCopyAsyncData, 1);
  data->file = g_object_ref (source);
  data->progress_cb = progress_callback;
  data->progress_cb_data = progress_callback_data;

  result = g_simple_async_result_new (G_OBJECT (source), callback, callback_data, g_file_deep_copy_async);
  g_simple_async_result_set_op_res_gpointer (result, data, (GDestroyNotify) deep_copy_async_data_free);

  g_io_scheduler_push_job (deep_copy_async_thread, result, g_object_unref, io_priority, cancellable);
}



static gboolean
g_file_deep_copy_finish (GFile        *file,
                          GAsyncResult *res,
                          GError      **error)
{
  g_return_val_if_fail (G_IS_FILE (file), FALSE);
...
static void
deep_copy_progress (goffset  current_num_files,
                     goffset  current_num_bytes,
                     gpointer user_data)
{
...
static void
deep_copy_finished (GObject      *source_object,
                     GAsyncResult *res,
                     gpointer      user_data)
{
  GError *error = NULL;

  if (g_file_deep_copy_finish (G_FILE (source_object), res, &error))
